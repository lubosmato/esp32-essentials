...
struct Mqtt {
  ...
  std::unique_ptr<Subscription> subscribe(std::string_view topic, Qos qos, std::function<void(std::optional<T>)> reaction) {
    return subscribe(topic, qos, [reaction](std::string_view data) {
      reaction(_fromString<T>(data));
    });
  }
...
private:
  ...
  static std::string _toString(T value) {
    ...
    if constexpr (std::is_same_v<T, bool>) {
      ...
      if (std::isinf(value))
        return std::string(value > 0.0 ? POSITIVE_INF_LITERAL : NEGATIVE_INF_LITERAL);
  ...
  static std::optional<T> _fromString(std::string_view textValue) {
    ...
    if constexpr (std::is_same_v<T, bool>) {
      ...
      else if (textValue == FALSE_LITERAL) return false;
      return std::nullopt;
    } else if constexpr (std::is_integral_v<T>) {
      ...
      std::optional<T> value;
      if (textValue == NAN_LITERAL) 
        return std::numeric_limits<T>::quiet_NaN();
        
      if (textValue == POSITIVE_INF_LITERAL)
        return std::numeric_limits<T>::infinity();
      
      if (textValue == NEGATIVE_INF_LITERAL)
        return -std::numeric_limits<T>::infinity();
